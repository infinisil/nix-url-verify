...
let
  ...
  getPos = p: if (tryEval p).success && p ? meta && (tryEval p.meta).success && p.meta ? position && p.meta.position != null then lib.removePrefix (toString ~/src/nixpkgs + "/") p.meta.position else null;
  getPosDes = input: let
...
map (name: { inherit name; path = getPos pkgs.${name}; urls = getSrc pkgs.${name}; }) (
